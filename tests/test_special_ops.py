...
def test_accuracy_grid_sampler_2d_extreme_grid(shape, dtype, mode, padding_mode):
    """Test with grid values far outside [-1, 1] to verify boundary handling."""
    inp, grid = _make_grid_sample_inputs_4d(shape, dtype, grid_range=3.0)

    if dtype == torch.bfloat16:
        # For bfloat16 with extreme grids, our kernel computes in float32
        # which is more precise than PyTorch CUDA's bfloat16 computation.
        # Use float32 reference to match our kernel's precision.
        ref_out = torch.nn.functional.grid_sample(
            inp.float(),
            grid.float(),
            mode=mode,
        ...
        # Compare against PyTorch CUDA (not CPU) to avoid CPU/CUDA precision diffs
        ref_out = torch.nn.functional.grid_sample(
            inp,
            grid,
            mode=mode,
...
def test_accuracy_grid_sampler_2d_nan_grid(dtype):
    """NaN values in grid should be treated as -1 (CUDA behavior)."""
    N, C, IH, IW = 1, 3, 8, 8
    OH, OW = 4, 4
    inp = torch.randn(N, C, IH, IW, device=device, dtype=dtype)
    grid = torch.randn(N, OH, OW, 2, device=device, dtype=dtype)
    # Replace NaN grid values with -1 to get the expected reference
    grid_nan = grid.clone()
